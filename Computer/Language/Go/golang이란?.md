...
>[goroutine](golang이란?.md#goroutine)

- 사용자 공간 기반의 경량 스케줄링
	- [Go 런타임](golang이란?.md#Go%20런타임) 은 커널 영역이 아닌 사용자 공간에 존재하며, 하나의 실행 파일에 빌드되어 포함된다.
...
- [goroutine](golang이란?.md#goroutine)이 생성될 때 2KB 크기의 아주 작은 스택을 가짐
...
- Go 런타임은 내부에 OS별 비동기 I/O 시스템(Linux `epoll`, macOS `kqueue`, Windows `IOCP`)을 추상화한 [netpoller](golang이란?.md#netpoller)를 둠
...
- 하나의 스레드 내부에서 [Multiplexing 구조](golang이란?.md#Multiplexing%20구조)을 사용하여 여러 가상 스레드를 처리
	- 기존 스레드보다 많은 양의 스레드를 생성할 수 있어서 효율적임.
- 기본적으로 1개의 CPU에서 처리하기 떄문에 동시성 (Concurrency) 임.
	-  만약 여러 개의 CPU를 사용하려면 (Parallel), `runtime.GOMAXPROCS(cpu개수)`([GOMAXPROCS](golang이란?.md#GOMAXPROCS))함수를 호출하여야한다. (`cpu개수`는 Logical CPU 수를 의미)
...
### 스케줄러 작동 방식
>[GMP Model](golang이란?.md#GMP%20Model)

![](../05_attachments/Pasted%20image%2020260709035249.png)
> Go Scheduler

![](../05_attachments/Pasted%20image%2020260709035319.png)
>1. `M`은 `P0`의 [Local Run Queue](golang이란?.md#Local%20Run%20Queue)에서 `G`를 가져와 실행한다.
>2. `G`의 실행이 종료되면, `M`은 `P0`의 [Local Run Queue](golang이란?.md#Local%20Run%20Queue)에 실행할 `G`가 있는지 확인한다.
>3. 만약 존재한다면, 가져와서 실행한다.
> 참고 : [P 구조체](golang이란?.md#P%20구조체)

![](../05_attachments/Pasted%20image%2020260709035650.png)
>4. 이제 모든 `G`를 실행하여 [Local Run Queue](golang이란?.md#Local%20Run%20Queue)가 비었다.
>5. 그렇다면, [Global Run Queue](golang이란?.md#Global%20Run%20Queue)에 실행할 `G`가 있는지 확인한다.
...
>7. 이제 [Global Run Queue](golang이란?.md#Global%20Run%20Queue)가 비었다.
>8. 그렇다면, [netpoller](golang이란?.md#netpoller)에 준비된 `G`가 있는지 확인한다.
>9. 만약 존재한다면, 가져와서 실행한다.

![](../05_attachments/Pasted%20image%2020260709040703.png)
>10. 이제 [netpoller](golang이란?.md#netpoller)도 비었다.
>11. 그렇다면, 이제부터는 랜덤하게 `P`를 하나를 선택한다.
>12. 그리고 선택한 `P`의 [Local Run Queue](golang이란?.md#Local%20Run%20Queue)의 **반을 가져온다.**
>13. 가져온 `G`를 실행한다.
>참고 : [워크 스틸링 (Work Stealing)](#워크%20스틸링%20(Work%20Stealing))
...
어떤 `G`의 실행 시간이 10ms가 넘으면, 해당 `G`를 실행하고 있는 `M`에게 선점 signal을 보낸다. **선점 signal을 받은 `M`은 `G`를 [Global Run Queue](golang이란?.md#Global%20Run%20Queue)에 보내게 된다.**
...
locality 문제를 해결하가 위해 Go 스케줄러는 `G`가 만들어지면, [Local Run Queue](golang이란?.md#Local%20Run%20Queue)의 tail 대신에 head에 넣는다. (`runnext` 라는 별도의 독립된 포인터에 들어간다. `M`이 `G`를 꺼낼 때 `runnext` -> `local queue` 순서로 가져감)
...
여기까지 이 방법으로 local runqueue의 기아 상태는 해결하겠지만, [Global Run Queue](golang이란?.md#Global%20Run%20Queue)의 기아 상태는 해결하지 못한다. `M`이 [Global Run Queue](golang이란?.md#Global%20Run%20Queue)의 `G`를 가져올 때는 [Local Run Queue](golang이란?.md#Local%20Run%20Queue)가 비었을 때다. **즉, local runqueue에 항상 `G`가 존재한다면, global runqueue의 `G`를 절대 가져오지 않는다.**
...
#### schedtick
>[Global Run Queue](golang이란?.md#Global%20Run%20Queue)의 기아 상태를 방지하기 위해 Go 런타임은 일정 주기마다 반드시 Global Run Queue를 polling해 `G`를 가져온다.

go runtime에는 `schedtick`이라는 변수가 존재하며, 이 변수는 [Local Run Queue](golang이란?.md#Local%20Run%20Queue)의 **polling**이 한 번 일어날 때마다 증가한다.

polling은 어떤 장치나 프로그램의 상태를 주기적으로 살핀다는 의미이며, 여기서는 `G` 하나를 수행하고 나서 다음으로 실행할 `G`를 찾기 위해 runqueue를 살피는 것을 뜻한다.

[Global Run Queue](golang이란?.md#Global%20Run%20Queue)를 반드시 polling하게 되는 조건은 다음과 같다.
...
Go scheduler는 **`P`를 다른 `M`에게 건내주어([Hand-off Mechanism](golang이란?.md#Hand-off%20Mechanism)) 이를 해결한다.**
...
**하지만 짧은 시간이 걸린다고 예측한 system call이 여러 사유에 의해 오래 걸릴 수 있다. 이런 경우에는 지켜보고 있던 [sysmon](golang이란?.md#sysmon)이 handoff를 실행한다.**
...
- 이를 해결하기 위해 Go 런타임은 다음과 같이 작동함.
	- 1. 커널 스레드 (M1)이 시스템 콜로 멈추기 직전, 자기가 가지고 있던 [P](golang이란?.md#P%20구조체)를 방출(Hand-off)함.
	- 2. 대기하고 있던 다른 스레드(M2)를 깨우거나 새로 만들어 방출한 [P](golang이란?.md#P%20구조체)와 결합.
	- 3. M2는 [P](golang이란?.md#P%20구조체)의 로컬 큐에 남아있던 다른 goroutine들을 멈추지 않고 계속 실행.
	- 4. 나중에 시스템 콜을 마친 M1이 돌아와 빈 [P](golang이란?.md#P%20구조체)가 있는지 찾고, 없다면 자신이 실행하던 G1을 글로벌 큐에 넣은 뒤 자신의 스레드 풀로 돌아가 대기함.
...
- ***P***는 하나의 ***M***에 할당된다. 따라서 ***P***는 [GOMAXPROCS](golang이란?.md#GOMAXPROCS) 의 개수와 같다.
	- ***M***은 ***G***를 실행하기 위해 ***P***를 참조하게 된다.
	- [워크 스틸링 (Work Stealing)](golang이란?.md#워크%20스틸링%20(Work%20Stealing)) 을 위해 체크해봐야 할 스레드의 개수가 무한하다는 문제를 ***P***구조체를 사용함으로써 ***P***의 개수가 [GOMAXPROCS](golang이란?.md#GOMAXPROCS) 와 같기 때문에 유한하게 되었으므로 해결됨 (***P***들만 체크하면 된다)
- [Local Run Queue](golang이란?.md#Local%20Run%20Queue) 뿐만 아니라 코드를 실행하기 위해 ***M***이 유지하고 있던 context도 모두 ***P***에 저장한다.
- ***P*** 구조체를 사용함으로써 block된 ***M***이 ***P***를 다른 ***M***에게 [Local Run Queue](golang이란?.md#Local%20Run%20Queue)과 함께 넘겨줄 수 있다. 
...
- 모든 ***P*** 는 자신만의 로컬 큐를 가지고 있으며, 여기에 실행 대기 중인 ***G*** 들을 보관 (참고 : [P 구조체](golang이란?.md#P%20구조체))
